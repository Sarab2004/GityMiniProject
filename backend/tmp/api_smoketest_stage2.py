...
# 1) ورود با سوپریوزر (اگر نیست بساز)
...
if not admin.is_superuser or not admin.is_staff:
    admin.is_superuser = True; admin.is_staff = True; admin.set_password("Xx_ap1_!234")
    admin.save()
...
# 3) GET list + search روی display_name/username
...
print("SEARCH:", resp_search.status_code, "", "PASS" if ok_search else "FAIL")
assert ok_search, "search failed"

# 4) PATCH فقط display_name و permissions (استراتژی replace)
patch_payload = {
...
# 5) GET permissions و سپس PUT permissions (replace)
...
put_perms = [
    {"resource":"archive","can_read":True,"can_create":False,"can_update":False,"can_delete":False}
]
...
print("PUT /permissions:", resp_put_perms.status_code, "", "PASS" if ok_put_perms else "FAIL")
assert ok_put_perms, "put permissions failed"

# 6) DELETE نرم (soft): is_active=False
resp_del_soft = client.delete(f"{BASE}{uid}/")
ok_del_soft = resp_del_soft.status_code in (200,204)
print("DELETE (soft):", resp_del_soft.status_code, "", "PASS" if ok_del_soft else "FAIL")
assert ok_del_soft, "soft delete failed"

# بررسی soft
resp_after = client.get(BASE, {"search":"apit_u1"})
if resp_after.status_code==200 and resp_after.json():
    item = resp_after.json()[0]
    print("VERIFY soft is_active:", "PASS" if (item.get("is_active") is False) else "WARN (not shown or still active)")
...
print("DELETE (hard):", resp_del_hard.status_code, "", "PASS" if ok_del_hard else "FAIL")
